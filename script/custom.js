...
var myFullpage = new fullpage('#fullpage', {
  ...
  /* ✅ 핵심 수정 */
  scrollOverflow: false,  // true면 section3이 조금만 넘쳐도 내부 스크롤 생기고 풀페이지 꼬임
  normalScrollElements: '#section3 .art_swiper_wrap, #section3 .swiper, #section3 .swiper *',
  ...
  if (destination.anchor === 'anchor1') {
    const v = document.querySelector('#section1 video');
    if (v) {
      v.muted = true;          // autoplay 정책 대비
      v.playsInline = true;
      v.currentTime = 0;       // "돌아올 때 처음부터" 원하면 유지 / 싫으면 이 줄 제거
      const p = v.play();
  ...
  afterRender: function () {
    ...
    if (activeSection && activeSection.id === 'section5') animateResumeBoxes('section5');
  }
});
...
/* =========================
   ✅ 여기! fullpage 스크롤 잠금 코드
========================= */
function lockFullpageScroll(lock){
  if (!window.fullpage_api) return;
  fullpage_api.setAllowScrolling(!lock);
  fullpage_api.setKeyboardScrolling(!lock);
}

document
  .querySelectorAll('#section3 .art_swiper, #section3 .art_swiper_wrap')
  .forEach(el => {

    // PC
    el.addEventListener('mouseenter', () => lockFullpageScroll(true));
    el.addEventListener('mouseleave', () => lockFullpageScroll(false));

    // Mobile
    el.addEventListener('touchstart', () => lockFullpageScroll(true), { passive: true });
    el.addEventListener('touchend', () => lockFullpageScroll(false));
});
...
$(function () {
  ...
  $("#slide-open").on("click", function () {
    ...
    $("#burgur_gnb").toggleClass("on");
  });

  /* ===============================
     메뉴 클릭 시 무조건 닫기
  =============================== */
  $("#burgur_gnb ul li a").on("click", function () {
    $("#burgur").removeClass("on");
    $("#burgur_gnb").removeClass("on");
  });
